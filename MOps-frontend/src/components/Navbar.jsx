            ...
            <nav className="fixed top-0 left-0 right-0 bg-white border-b border-outline/20 z-50 h-[64px]">
                <div className="max-w-[1440px] mx-auto h-full px-6 flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-2 group">
                        <span className="text-[24px] font-bold font-display text-secondary tracking-tight group-hover:text-primary transition-colors">
                            NVCC <span className="font-medium text-primary">MaintenOps</span>
                        </span>
                    </Link>
                        ...
                        {user ? (
                            <>
                                <Link
                                    ...
                                    className="text-[14px] font-semibold text-on-surface-variant font-ui hover:text-primary transition-colors"
                                >
                                    Dashboard
                                </Link>

                                {/* User Profile Dropdown */}
                                <div className="relative flex items-center pl-4 border-l border-outline" ref={dropdownRef}>
                                    <button
                                        onClick={() => setDropdownOpen(!dropdownOpen)}
                                        className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-bold text-[14px] shadow-sm hover:ring-2 hover:ring-primary-container transition-all"
                                        title="Profile"
                                    >
                                    ...
                                    {/* Dropdown Menu */}
                                    {dropdownOpen && (
                                        <div className="absolute top-[52px] right-0 w-[240px] bg-white rounded-md shadow-lg border border-outline py-2 animate-fadeUp origin-top-right">
                                            <div className="px-4 py-3 border-b border-outline mb-2">
                                                ...
                                                <div className="text-[13px] font-body text-on-surface-variant truncate">
                                                    {user?.email || 'hitesh.kumar@example.com'}
                                                </div>
                                                <div className="mt-2 inline-flex items-center px-3 py-0.5 rounded-pill bg-primary-container text-primary text-[11px] font-semibold tracking-wide border border-primary/20">
                            ...
                            <>
                                <Link to="/login">
                                    <button className="h-10 px-6 rounded-md border-[1.5px] border-outline text-on-surface-variant font-ui text-[14px] font-semibold hover:text-on-surface hover:bg-surface-variant transition-all duration-200">
                                        Log in
                                    </button>
                                </Link>
                                <Link to="/signup">
                                    <button className="h-10 px-6 rounded-md bg-primary text-white font-ui text-[14px] font-semibold shadow-md hover:filter hover:brightness-110 transition-all duration-200 scale-100 active:scale-95">
                                        Sign up
                                    </button>
                                </Link>
                            </>
            ...
            {/* Logout Confirmation Modal */}
            {
                showLogoutConfirm && (
                    <div className="fixed inset-0 bg-black/40 backdrop-blur-[4px] z-[100] flex items-center justify-center p-4 animate-fadeUp">
                        <div className="bg-white rounded-lg w-full max-w-[400px] p-6 shadow-lg border border-outline">
                            <h3 className="text-[22px] font-display font-bold text-secondary mb-2">Sign out</h3>
                            <p className="text-[14px] font-body text-on-surface-variant mb-6">
                                Are you sure you want to sign out of NVCC MaintenanceOps?
                            </p>
                            <div className="flex justify-end gap-3">
                                <button
                                    onClick={() => setShowLogoutConfirm(false)}
                                    className="px-5 py-2 font-ui text-[14px] font-semibold text-on-surface-variant hover:bg-surface-variant rounded-md transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={handleLogout}
                                    className="px-6 py-2 rounded-md bg-error text-white font-ui text-[14px] font-semibold shadow-md hover:filter hover:brightness-110 transition-colors"
                                >
                                    Sign out
                                </button>
                            </div>
